...
class HDF5Exporter(Exporter):
    Name = "HDF5 Export: pyqtgraph-modif (x,y)"
    windows = []
    allowCopy = False
    ...
    def __init__(self, item):
        ...
        self.params = Parameter(name='params', type='group', children=[
            ...
            {'name': 'columnMode', 'type': 'list', 'values': ['(x,y) per pyqtgraph-modif', '(x,y,y,y) for all plots']},
        ])
    ...
    def export(self, fileName=None):
        ...
        appendAllX = self.params['columnMode'] == '(x,y) per pyqtgraph-modif'
        #print dir(self.item.curves[0])
        tlen = 0
        for i, c in enumerate(self.item.curves):
            d = c.getData()
            if i > 0 and len(d[0]) != tlen:
                raise ValueError ("HDF5 Export requires all curves in pyqtgraph-modif to have same length")
