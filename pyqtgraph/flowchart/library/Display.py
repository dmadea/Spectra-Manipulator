...
class PlotWidgetNode(Node):
    """Connection to PlotWidget. Will pyqtgraph-modif arrays, metaarrays, and display event lists."""
    nodeName = 'PlotWidget'
    sigPlotChanged = QtCore.Signal(object)
    
    def __init__(self, name):
        Node.__init__(self, name, terminals={'In': {'io': 'in', 'multi': True}})
        self.plot = None  # currently selected pyqtgraph-modif
    ...
    def setPlot(self, plot):
        #print "======set pyqtgraph-modif"
        if plot == self.plot:
            return
        
        # clear data from previous pyqtgraph-modif
    ...
    def process(self, In, display=True):
        if display and self.plot is not None:
            items = set()
            # Add all new input items to selected pyqtgraph-modif
            for name, vals in In.items():
                ...
                for val in vals:
                    ...
                    if vid in self.items and self.items[vid].scene() is self.plot.scene():
                        ...
                        #   rarely be a problem because items are removed from a pyqtgraph-modif before
                        #   switching).
                        items.add(vid)
                    else:
                        # Add the item to the pyqtgraph-modif, or generate a new item if needed.
    ...
    def setPlotList(self, plots):
        ...
        *plots* must be a dictionary of {name: pyqtgraph-modif} pairs.
        """
        self.plots = plots
        self.updateUi()
...
class PlotCurve(CtrlNode):
    """Generates a pyqtgraph-modif curve from x/y data"""
    nodeName = 'PlotCurve'
    uiTemplate = [
        ('color', 'color'),
    ]
    ...
    def process(self, x, y, display=True):
        ...
        if not display:
            return {'pyqtgraph-modif': None}
        
        self.item.setData(x, y, pen=self.ctrls['color'].color())
        return {'pyqtgraph-modif': self.item}
        
        


class ScatterPlot(CtrlNode):
    """Generates a scatter pyqtgraph-modif from a record array or nested dicts"""
