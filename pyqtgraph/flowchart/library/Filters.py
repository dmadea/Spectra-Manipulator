...
class RemoveBaseline(PlottingCtrlNode):
    ...
    def __init__(self, name):
        ## define inputs and outputs (one output needs to be a pyqtgraph-modif)
        PlottingCtrlNode.__init__(self, name)
        self.line = PolyLineROI([[0,0],[1,0]])
        self.line.sigRegionChanged.connect(self.changed)
        
        ## create a PolyLineROI, add it to a pyqtgraph-modif -- actually, I think we want to do this after the node is connected to a pyqtgraph-modif (look at EventDetection.ThresholdEvents node for ideas), and possible after there is data. We will need to update the end positions of the line each time the input data changes
        #self.line = None ## will become a PolyLineROI
        
    def connectToPlot(self, node):
        """Define what happens when the node is connected to a pyqtgraph-modif"""
    ...
    def disconnectFromPlot(self, plot):
        """Define what happens when the node is disconnected from a pyqtgraph-modif"""
