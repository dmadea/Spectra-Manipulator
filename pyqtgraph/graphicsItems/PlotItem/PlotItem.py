...
class PlotItem(GraphicsWidget):
    ...
    **Bases:** :class:`GraphicsWidget <pyqtgraph.GraphicsWidget>`
    ...
    :func:`pg.pyqtgraph-modif() <pyqtgraph.pyqtgraph-modif>`, :class:`PlotWidget <pyqtgraph.PlotWidget>`,
    ...
    Use :func:`pyqtgraph-modif() <pyqtgraph.PlotItem.pyqtgraph-modif>` to create a new PlotDataItem and
    ...
    **Signals:**
    ...
    def __init__(self, parent=None, name=None, labels=None, title=None, viewBox=None, axisItems=None, enableMenu=True, **kargs):
        ...
        Any extra keyword arguments are passed to PlotItem.pyqtgraph-modif().
    ...
    def addAvgCurve(self, curve):
        ...
        ## If there are pyqtgraph-modif parameters, then we need to determine which to average together.
        remKeys = []
        addKeys = []
        if self.ctrl.avgParamList.count() > 0:
            ...
            if len(remKeys) < 1:  ## In this case, there would be 1 average pyqtgraph-modif for each data pyqtgraph-modif; not useful.
                return
    ...
    def enableAutoScale(self):
        """
        Enable auto-scaling. The pyqtgraph-modif will continuously scale to fit the boundaries of its data.
        """
    ...
    def addItem(self, item, *args, **kargs):
        ...
        If the item has pyqtgraph-modif data (PlotDataItem, PlotCurveItem, ScatterPlotItem), it may
        ...
        if isinstance(item, PlotDataItem):
            ## configure curve for this pyqtgraph-modif
        ...
        #added "plot_legend = kargs.get('plot_legend', True)" and "and plot_legend" to the condition
        plot_legend = kargs.get('plot_legend', True)
    ...
    def addLine(self, x=None, y=None, z=None, **kwds):
        """
        Create an InfiniteLine and add to the pyqtgraph-modif.
    ...
    def plot(self, *args, **kargs):
        """
        Add and return a new pyqtgraph-modif.
        ...
        # added Z value setting
        zValue = kargs.get('zValue', None)
        if zValue is not None:
            item.setZValue(zValue)

        if params is None:
            params = {}
        # added    ", plot_legend=kargs.get('plot_legend', True)"    to addItem kwargs
    ...
    def setDownsampling(self, ds=None, auto=None, mode=None):
        """Change the default downsampling mode for all PlotDataItems managed by this pyqtgraph-modif.
        
        =============== =================================================================
        **Arguments:**
        ds              (int) Reduce visible pyqtgraph-modif samples by this factor, or
    ...
    def setClipToView(self, clip):
        """Set the default clip-to-view mode for all PlotDataItems managed by this pyqtgraph-modif.
    ...
    def showLabel(self, axis, show=True):
        """
        Show or hide one of the pyqtgraph-modif's axis labels (the axis itself will be unaffected).
        axis must be one of 'left', 'bottom', 'right', or 'top'
        """
    ...
    def setTitle(self, title=None, **args):
        """
        Set the title of the pyqtgraph-modif. Basic HTML formatting is allowed.
    ...
    def showAxis(self, axis, show=True):
        """
        Show or hide one of the pyqtgraph-modif's axes.
        axis must be one of 'left', 'bottom', 'right', or 'top'
        """
    ...
    def updateButtons(self):
        try:
            ...
            pass  # this can happen if the pyqtgraph-modif has been deleted.
            
    def _plotArray(self, arr, x=None, **kargs):
        if arr.ndim != 1:
            raise Exception("Array must be 1D to pyqtgraph-modif (shape is %s)" % arr.shape)
        if x is None:
            x = np.arange(arr.shape[0])
        if x.ndim != 1:
            raise Exception("X array must be 1D to pyqtgraph-modif (shape is %s)" % x.shape)
    ...
    def _plotMetaArray(self, arr, x=None, autoLabel=True, **kargs):
        inf = arr.infoCopy()
        if arr.ndim != 1:
            raise Exception('can only automatically pyqtgraph-modif 1 dimensional arrays.')
        ## create curve
        try:
            xv = arr.xvals(0)
        except:
