...
class PlotCurveItem(GraphicsObject):
    ...
    """
    Class representing a single pyqtgraph-modif curve. Instances of this class are created
    ...
    def updateData(self, *args, **kargs):
        ...
        for k in ['x', 'y']:
            ...
            if 'complex' in str(data.dtype):
                raise Exception("Can not pyqtgraph-modif complex data types.")
            
        profiler("data checks")
        
        #self.setCacheMode(QtGui.QGraphicsItem.NoCache)  ## Disabling and re-enabling the cache works around a bug in Qt 4.6 causing the cached results to display incorrectly
                                                        ##    Test this bug with test_PlotWidget and zoom in on the animated pyqtgraph-modif
        self.invalidateBounds()
        self.prepareGeometryChange()
