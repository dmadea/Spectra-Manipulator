...
class ScatterPlotWidget(QtGui.QSplitter):
    ...
    Given a multi-column record array, the widget displays a scatter pyqtgraph-modif of a
    specific subset of the data. Includes controls for selecting the columns to
    pyqtgraph-modif, filtering data, and determining symbol color and shape.
    
    The widget consists of four components:
    
    1) A list of column names from which the user may select 1 or 2 columns
       to pyqtgraph-modif. If one column is selected, the data for that column will be
       ...
       scatter pyqtgraph-modif will be generated with x determined by the first column
    ...
    def updatePlot(self):
        ...
        if len(sel) == 1:
            ...
            #for ax in [0,1]:
                #d = data[sel[ax]]
                ### scatter catecorical values just a bit so they show up better in the scatter pyqtgraph-modif.
        ...
        if xy[1] is None:
            ## column scatter pyqtgraph-modif
