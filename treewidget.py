from PyQt5.QtCore import Qt, QItemSelectionModel, QItemSelection, pyqtSignal
...
class TreeWidget(TreeView):
    ...
    # returns a special ItemList class that enables simple calculation with spectra that contains...
    def get_hierarchic_list(self, items_iterator):
    ...
    def copy_selected_spectra_to_clipboard(self):

        sp_list = self.get_hierarchic_list(self.myModel.iterate_selected_items(skip_groups=True,
                                                                               skip_childs_in_selected_groups=False))

        if len(sp_list) == 0:
            return

        Logger.status_message("Copying selected spectra to clipboard...")
        try:

            output = Spectrum.list_to_string(sp_list, include_group_name=Settings.clip_exp_include_group_name,
    ...
    def paste_from_clipboard(self):
        ...
        spectra = dataparser.parse_text(text_data)
    ...
    def normalize(self):
        ...
        rng_dialog = RangeWidget(UserNamespace.instance.main.var_widget, accepted, title="Normalize",
    ...
    def cut(self):
        ...
        rng_dialog = RangeWidget(UserNamespace.instance.main.var_widget, accepted, title="Cut",
    ...
    def extend_by_zeros(self):
        ...
        rng_dialog = RangeWidget(UserNamespace.instance.main.var_widget, accepted, title="Extend by zeros",
    ...
    def baseline_correct(self):
        ...
        rng_dialog = RangeWidget(UserNamespace.instance.main.var_widget, accepted, title="Baseline correction",
    ...
    def fit_curve(self):
        ...
        fit_dialog = FitWidget(UserNamespace.instance.main.var_widget, accepted, selected_node, parent=self)
    ...
    def set_style(self):
        ...
        style_widget = StyleWidget(UserNamespace.instance.main.var_widget, accepted, selected_node, parent=self)
    ...
    def context_menu(self, pos):
        ...
        copy_to_clipboard = menu.addAction("Copy Selected Spectra To Clipboard (Ctrl + C)")
        copy_to_clipboard.triggered.connect(self.copy_selected_spectra_to_clipboard)
    ...
    def parse_XML_Spreadsheet(self, byte_data):
        ...
        try:
            ...
            spectra = dataparser.parse_XML_Spreadsheet(xml_data)
            self.import_spectra(spectra)
        except Exception as ex:
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == 67 and e.modifiers() == Qt.ControlModifier:
            # Ctrl + C
            self.copy_selected_spectra_to_clipboard()
    ...
    def import_files(self, filepaths):
        ...
        if filepaths[0].lower().endswith(".smpj"):
            self.parent().parent().open_project(filepaths[0], open_dialog=False)
            return

        spectra = dataparser.parse_files(filepaths)
        self.import_spectra(spectra)
